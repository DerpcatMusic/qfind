...
fn parse_osc72(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    let body = buffer
        .strip_prefix(b"\x1b]72;")
        .and_then(|body| body.strip_suffix(b"\x1b\\"))
        .ok_or_else(could_not_parse_event_error)?;
    let meta = body.split(|byte| *byte == b';').next().unwrap_or_default();
    let mut kind = 0;
    let mut x = None;
    let mut y = None;
    for field in meta.split(|byte| *byte == b':') {
        let Some(at) = field.iter().position(|byte| *byte == b'=') else {
            continue;
        };
        let (key, value) = (&field[..at], &field[at + 1..]);
        match key {
            b"t" => kind = value.first().copied().unwrap_or_default(),
            b"x" => x = std::str::from_utf8(value).ok().and_then(|v| v.parse().ok()),
            b"y" => y = std::str::from_utf8(value).ok().and_then(|v| v.parse().ok()),
            _ => {}
        }
    }
    Ok(Some(InternalEvent::Event(Event::Osc72 { kind, x, y })))
}

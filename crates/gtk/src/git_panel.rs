...
pub fn new(
    ...
    project: Option<Rc<RefCell<Option<PathBuf>>>>,
) -> (gtk::Box, gtk::Button) {
    ...
    controls.append(&expand);
    {
        ...
        expand.connect_clicked(move |button| {
            let (Some(host), Some(root)) = (host.upgrade(), root.upgrade()) else {
                return;
            };
            let window = gtk::Window::builder()
                .title("Megaman · Changes")
                .default_width(1100)
                .default_height(760)
                .build();
    ...
    copy.connect_clicked(move |button| {
        button.display().clipboard().set_text(&patch.text(
            &patch.start_iter(),
            &patch.end_iter(),
            false,
        ))
    ...
    let action_busy = Rc::new(Cell::new(false));
    {
        ...
        let (state, project, mode, file_picker, file_names, refresh, action_status, busy) = (
            state.clone(),
            project.clone(),
            mode.clone(),
            file_picker.clone(),
        ...
        stage.connect_clicked(move |button| {
            ...
            let Some(file) = (file_picker.selected() > 0)
                .then(|| file_names.string(file_picker.selected()))
                .flatten()
            else {
                busy.set(false);
                return;
            ...
            if !reviewed.borrow().as_ref().is_some_and(|query| {
                query.directory == directory
                    && query.staged == staged
                    && query.file.as_deref() == Some(Path::new(file.as_str()))
                    && query.visible
            }) {
                busy.set(false);
            ...
            let (button, status, refresh, busy) = (
                button.clone(),
                action_status.clone(),
                refresh.clone(),
                busy.clone(),
            ...
            glib::MainContext::default().spawn_local(async move {
                let result = gio::spawn_blocking(move || {
                    let root = PathBuf::from(
                        git(&directory, &["rev-parse", "--show-toplevel"], None)?.trim(),
                    );
                    ...
                    if staged {
                        if git(&root, &["rev-parse", "--verify", "HEAD"], None).is_ok() {
                            ...
                            git(&root, &["rm", "--cached"], Some(path))
                        }
                    } else {
                        git(&root, &["add"], Some(path))
                    }
                })
                .await;
                busy.set(false);
                match result {
                    Ok(Ok(_)) => status.set_text(if staged {
                        "File unstaged"
                    } else {
                        "File staged"
                    }),
    ...
    }
    {
        ...
        branches.connect_clicked(move |_| {
            ...
            glib::MainContext::default().spawn_local(async move {
                ...
                .await
                .unwrap_or_else(|_| "Branch listing failed".into());
    ...
    glib::timeout_add_local(Duration::from_millis(200), move || {
        ...
        if !root.is_mapped() && !footer_poll.is_mapped() {
            return glib::ControlFlow::Continue;
        }
        if project.as_ref().is_some_and(|path| path.borrow().is_none()) {
            return glib::ControlFlow::Continue;
        }
        if project.is_some() && !root.is_mapped() {
            return glib::ControlFlow::Continue;
        }
        stage.set_label(if mode.selected() == 1 {
            "Unstage file"
        } else {
            "Stage file"
        });
        stage.set_sensitive(
            file_picker.selected() > 0
                && file_picker.selected() < file_names.n_items()
                && !action_busy.get(),
        );
        ...
        let query = Query {
            directory,
            selected: if project.is_none() && root.is_mapped() {
                selected_path(&state)
            } else {
                None
            },
            staged: mode.selected() == 1,
            file: (file_picker.selected() > 0)
                .then(|| file_names.string(file_picker.selected()))
                .flatten()
        ...
        if last.as_ref() == Some(&query)
            && !refresh_requested.get()
            && last_status.elapsed() < Duration::from_secs(5)
        {
            return glib::ControlFlow::Continue;
        }
...
fn word_changes(left: &str, right: &str, budget: &mut usize) -> [Vec<(i32, i32)>; 2] {
    if left.len() > 2048 || right.len() > 2048 {
        return [vec![], vec![]];
    }
    let tokens = |text: &str| {
        ...
        for (byte, ch) in text.char_indices() {
            ...
            if category.is_some_and(|old| old != kind || kind == 2) {
                result.push((start, byte, token_start, chars));
                start = byte;
                token_start = chars;
            }
            category = Some(kind);
        ...
        if start < text.len() {
            result.push((start, text.len(), token_start, chars));
        }
        result
    };
    let (a, b) = (tokens(left), tokens(right));
    let work = (a.len() + 1).saturating_mul(b.len() + 1);
    if a.len() > 128 || b.len() > 128 || work > *budget {
        return [vec![], vec![]];
    }
    *budget -= work;
    let width = b.len() + 1;
    ...
    for i in (0..a.len()).rev() {
        for j in (0..b.len()).rev() {
            lcs[i * width + j] = if left[a[i].0..a[i].1] == right[b[j].0..b[j].1] {
                1 + lcs[(i + 1) * width + j + 1]
            } else {
    ...
    while i < a.len() || j < b.len() {
        if i < a.len() && j < b.len() && left[a[i].0..a[i].1] == right[b[j].0..b[j].1] {
            i += 1;
            j += 1;
        } else if i < a.len()
            && (j == b.len() || lcs[(i + 1) * width + j] >= lcs[i * width + j + 1])
        {
            changed[0].push((a[i].2, a[i].3));
            i += 1;
        } else {
            changed[1].push((b[j].2, b[j].3));
            j += 1;
        }
    }
    changed
...
fn diff_view(source: &gtk::TextBuffer) -> gtk::Box {
    ...
    let left = gtk::TextView::builder()
        .editable(false)
        .cursor_visible(false)
        .monospace(true)
        .build();
    let right = gtk::TextView::builder()
        .editable(false)
        .cursor_visible(false)
        .monospace(true)
        .build();
    ...
    for view in [&left, &right] {
        for (name, color) in [("add", "#57b879"), ("del", "#e47e87")] {
            view.buffer()
                .tag_table()
                .add(&gtk::TextTag::builder().name(name).foreground(color).build());
        }
        view.buffer().tag_table().add(
            &gtk::TextTag::builder()
                .name("word")
                .background_rgba(&gtk::gdk::RGBA::new(0.6, 0.6, 0.25, 0.30))
                .weight(700)
                .build(),
        );
        view.buffer()
            .tag_table()
            .add(&gtk::TextTag::builder().name("header").weight(700).build());
    }
    let render: Rc<dyn Fn()> = {
        let (collapsed, hunk_rows, hunk_names, hunk_picker, fold) = (
            collapsed.clone(),
            hunk_rows.clone(),
            hunk_names.clone(),
            hunk_picker.downgrade(),
            fold.downgrade(),
        );
        let (left, right, left_scroll, right_scroll, source, split, context) = (
            left.clone(),
            right.clone(),
            left_scroll.clone(),
            right_scroll.clone(),
        ...
        Rc::new(move || {
            let (Some(split), Some(context)) = (split.upgrade(), context.upgrade()) else {
                return;
            };
            ...
            let saved: Vec<_> = buffers
                .iter()
                .map(|b| b.selection_bounds().map(|(a, z)| (a.offset(), z.offset())))
                .collect();
            ...
            let (mut removed, mut added) =
                (Vec::<(String, i32)>::new(), Vec::<(String, i32)>::new());
            ...
            let flush = |removed: &mut Vec<(String, i32)>, added: &mut Vec<(String, i32)>| {
                for i in 0..removed.len().max(added.len()) {
                    let mut offsets = [0, 0];
                    for (side, (lines, tag)) in
                        [(removed.as_slice(), "del"), (added.as_slice(), "add")]
                            .into_iter()
                            .enumerate()
                    {
                        if split.is_active() {
                            offsets[side] = buffers[side].char_count();
                            buffers[side].insert_with_tags_by_name(
                                &mut buffers[side].end_iter(),
                                lines.get(i).map(|row| row.0.as_str()).unwrap_or("\n"),
                                &[tag],
                            );
                        } else {
                            offsets[side] = lines.get(i).map(|row| row.1).unwrap_or(0);
                        }
                    }
                    if let (Some(a), Some(b)) = (removed.get(i), added.get(i)) {
                        ...
                        let gutters = [&a.0, &b.0].map(|line| {
                            line.split_once("│ ")
                                .map(|(prefix, _)| {
                                    prefix.chars().count() as i32
                                        + 2
                                        + i32::from(!split.is_active())
                                })
                                .unwrap_or(0)
                        });
                        let (left, right) = if split.is_active() {
                            (left, right)
                        } else {
                            (&left[1..], &right[1..])
                        };
                        ...
                        for (side, spans) in changes.into_iter().enumerate() {
                            ...
                            for (a, b) in spans {
                                buffer.apply_tag_by_name(
                                    "word",
                                    &buffer.iter_at_offset(offsets[side] + gutters[side] + a),
                                    &buffer.iter_at_offset(offsets[side] + gutters[side] + b),
                                );
                            }
                        }
                    }
            ...
            for line in value.lines() {
                ...
                let kind = if in_hunk && !header {
                    line.chars().next().unwrap_or(' ')
                } else {
                    '\0'
                };
                let old_number = if matches!(kind, '-' | ' ') {
                    let n = old;
                    old += 1;
                    format!("{n:>5}")
                } else {
                ...
                let new_number = if matches!(kind, '+' | ' ') {
                    let n = new;
                    new += 1;
                    format!("{n:>5}")
                } else {
                ...
                if in_hunk && !header && collapsed.borrow().contains(&current_hunk) {
                    continue;
                }
                if kind == ' ' && !context.is_active() {
                    continue;
                }
                let folded_header =
                    if line.starts_with("@@ ") && collapsed.borrow().contains(&current_hunk) {
                        Some(format!("{line}  [collapsed]"))
                    } else {
                        None
                    };
                ...
                if split.is_active() {
                    let content = if matches!(kind, '+' | '-' | ' ') {
                        &line[1..]
                    } else {
                        line
                    };
                    if kind == '-' {
                        removed.push((format!("{old_number} │ {content}\n"), 0));
                    } else if kind == '+' {
                        added.push((format!("{new_number} │ {content}\n"), 0));
                    } else {
                    ...
                    if kind == '-' {
                        removed.push((row, offset));
                    } else if kind == '+' {
                        added.push((row, offset));
                    }
                }
            }
            flush(&mut removed, &mut added);
            let old_key = hunk_picker.upgrade().and_then(|picker| {
                hunk_rows
                    .borrow()
            ...
            collapsed
                .borrow_mut()
                .retain(|key| entries.iter().any(|row| &row.0 == key));
            *hunk_rows.borrow_mut() = entries;
            if (0..hunk_names.n_items())
                .filter_map(|i| hunk_names.string(i))
                .map(|s| s.to_string())
                .collect::<Vec<_>>()
                != labels
            {
                ...
                if let Some(picker) = hunk_picker.upgrade() {
                    picker.set_selected(
                        old_key
                            .and_then(|key| hunk_rows.borrow().iter().position(|row| row.0 == key))
                            .unwrap_or(0) as u32,
                    );
                }
            }
            if let (Some(picker), Some(fold)) = (hunk_picker.upgrade(), fold.upgrade()) {
                ...
                fold.set_label(
                    if current.is_some_and(|row| collapsed.borrow().contains(&row.0)) {
                        "Reveal"
                    } else {
                        "Collapse"
                    },
            ...
            glib::idle_add_local_once(move || {
                adjustment
                    .set_value(scroll.min((adjustment.upper() - adjustment.page_size()).max(0.0)))
            });
            for (buffer, selection) in buffers.iter().zip(saved) {
                if let Some((a, z)) = selection {
                    buffer.select_range(
                        &buffer.iter_at_offset(a.min(buffer.char_count())),
                        &buffer.iter_at_offset(z.min(buffer.char_count())),
                    );
                }
            }
        })
    ...
    let signal = source.connect_changed(move |_| {
    ...
    root.connect_destroy(move |_| {
        if let Some(signal) = signal.borrow_mut().take() {
            source_clone.disconnect(signal);
        }
    });
    for toggle in [&split, &context] {
        let render = render.clone();
        toggle.connect_toggled(move |_| render());
    }
    {
        ...
        wrap.connect_toggled(move |button| {
            ...
            left.set_wrap_mode(mode);
            right.set_wrap_mode(mode);
        });
    }
    {
        let (rows, collapsed, picker, render) = (
            hunk_rows.clone(),
            collapsed.clone(),
            hunk_picker.downgrade(),
            render.clone(),
    ...
    }
    {
        ...
        hunk_picker.connect_selected_notify(move |picker| {
            let Some(fold) = fold.upgrade() else {
                return;
            };
            ...
            if let Some((_, line)) = current {
                ...
                glib::idle_add_local_once(move || {
                    if let Some(mut iter) = left.buffer().iter_at_line(line) {
                        left.scroll_to_iter(&mut iter, 0.1, true, 0.0, 0.1);
                    }
                });
            }
        });
    ...
    for (button, direction) in [(previous, -1_i32), (next, 1)] {
        let (left, rows, collapsed, picker, render) = (
            left.clone(),
            hunk_rows.clone(),
            collapsed.clone(),
            hunk_picker.clone(),

...
fn all(view: &gtk::ColumnView) -> Vec<gtk::ColumnViewColumn> {
    (0..view.columns().n_items())
        .filter_map(|i| view.columns().item(i).and_downcast())
        .collect()
}
...
pub fn configure(view: &gtk::ColumnView, key: &str) -> gtk::MenuButton {
    ...
    let path = dirs::config_dir()
        .unwrap_or_default()
        .join(format!("qfind/columns-{key}"));
    let columns = all(view);
    let defaults: Vec<_> = columns
        .iter()
        .map(|c| (c.fixed_width(), c.is_visible(), c.expands()))
        .collect();
        ...
        if saved.starts_with("# columns-v2\n") {
            for (position, line) in saved.lines().skip(1).enumerate() {
                ...
                let Some(column) = columns
                    .iter()
                    .find(|c| c.title().as_deref() == Some(parts[3]))
                else {
                    continue;
                };
                if let Ok(width) = parts[0].parse::<i32>() {
                    column.set_fixed_width(width.clamp(-1, 10000));
                }
                ...
                column.set_visible(
                    index == 0
                        || (defaults[index].1
                            && !saved
                                .lines()
                                .any(|line| Some(line) == column.title().as_deref())),
                );
            }
        }
    }
        ...
        let source = glib::timeout_add_local_once(Duration::from_millis(350), move || {
            ...
            for column in all(&view) {
                text.push_str(&format!(
                    "{}\t{}\t{}\t{}\n",
                    column.fixed_width(),
                    u8::from(column.is_visible()),
                    u8::from(column.expands()),
                    column.title().unwrap_or_default()
                ));
            }
            ...
            if let Err(error) = result {
                eprintln!("Could not save column settings: {error}");
            }
        });
    ...
    for (index, column) in columns.iter().enumerate() {
        ...
        column.connect_visible_notify(move |column| {
            if let Some(action) = weak_action.upgrade() {
    ...
    view.columns()
        .connect_items_changed(move |_, _, _, _| save_order());
    ...
    reset.connect_activate(move |_, _| {
        ...
        for (index, (column, &(width, visible, expand))) in
            reset_columns.iter().zip(&defaults).enumerate()
        {
            view.remove_column(column);
...
pub fn text_factory(location: bool, zebra: Rc<Cell<bool>>) -> gtk::SignalListItemFactory {
    ...
    factory.connect_bind(move |_, item| {
        ...
        if zebra.get() && item.position() % 2 == 1 {
            label.add_css_class("qfind-odd");
        } else {
            label.remove_css_class("qfind-odd");
        }
    });
    factory
}
